...
def _(mo):
    ...
    - [GSE264667](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE264667)
      (Nadig et al. 2025) — dual-guide CRISPRi Perturb-seq in HepG2 and Jurkat (~408K cells)
    - [GSE153056](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE153056)
      (Papalexi et al. 2021) — ECCITE-seq in THP-1 (~38K cells, gene expression + protein abundance)
    - [GSE161002](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE161002)
      (Liscovitch-Brauer et al. 2021) — CRISPR-sciATAC in K-562 (~138K cells, chromatin accessibility)
    - [cpg0021-periscope](https://github.com/broadinstitute/cellpainting-gallery)
      (Feldman et al. 2025) — genome-wide CRISPRko Cell Painting in HeLa (~4.8K cells, image features + tiles)
...
def _(DB_URI, RaggedAtlas):
    ...
    atlas_rw = RaggedAtlas.checkout_latest(db_uri=DB_URI)
    atlas_rw
    return (atlas_rw,)
...
def _(mo):
    ...
    The fluent `atlas.query()` API returns an `AtlasQuery` builder;
    terminal methods like `.count()`, `.to_polars()`, and `.to_anndata()`
    execute the query.
    """)
    return
...
def _(atlas_rw):
    gene_registry = (
        atlas_rw._registry_tables["gene_expression"].search().to_polars().sort("global_index")
    )
...
def _(atlas_rw):
    protein_registry = (
        atlas_rw._registry_tables["protein_abundance"].search().to_polars().sort("global_index")
    )
...
def _(atlas_rw):
    ...
    multimodal = atlas_rw.query().balanced_limit(5000, "dataset_uid").to_multimodal()
    multimodal
    return (multimodal,)
...
def _(mo):
    ...
    The atlas stores perturbation assignments as parallel lists on each cell.
    We can filter for specific perturbation patterns using the
    `perturbation_search_string` column, which encodes
    `{type_prefix}:{uid}` tokens for full-text search.
    """)
    return
...
def _(dataset, make_loader, sampler, sparse_to_dense_collate, torch):
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        if batch_idx == 0:
            ...
            print(
                f"  Row sums (UMI counts): min={X.sum(1).min():.0f}, "
...
def _(atlas_rw, np):
    ...
    print(
        f"Per-cell: min={per_cell_counts.min()}, median={int(np.median(per_cell_counts))}, max={per_cell_counts.max()}"
    )
    ...
    single_cell_df = pl.DataFrame(
        {
            ...
            "end": frag_result.starts[s:e].astype(np.int64)
            + frag_result.lengths[s:e].astype(np.int64),
        }
    )
    print(
        f"Cell {cell_idx}: {e - s} fragments across {single_cell_df['chrom'].n_unique()} chromosomes"
    )
...
@app.cell
def _(frag_result):
    atac_pert_summary = frag_result.obs[["is_negative_control"]].value_counts().sort_index()
    atac_pert_summary
    return
...
def _(atlas_rw):
    image_feature_registry = (
        atlas_rw._registry_tables["image_features"].search().to_polars().sort("global_index")
    )
...
def _(atlas_rw):
    adata_image = (
        atlas_rw.query().where("cell_line = 'HeLa'").feature_spaces("image_features").to_anndata()
    )
    adata_image
    return (adata_image,)
...
def _(adata_image, np, pl, plt):
    ...
    for _ax, compartment in zip(axes_comp, compartments, strict=False):
...
def _(np, plt, tiles):
    ...
    for _c, (_ax, _name) in enumerate(zip(axes_ch, channel_names, strict=False)):
...
def _():
    ...
    lancedb.connect("s3://epiblast/ontology_resolver/").open_table("guide_rnas").to_pandas()
    return
