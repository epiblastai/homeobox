...
def _(mo):
    mo.md(r"""
    # TileDB-SOMA vs homeobox: Dataloader & Query Benchmark
...
def _(time):
    ...
    HOMEOBOX_DIR = "s3://epiblast-public/cellxgene_mouse_homeobox/"
    S3_KWARGS = {"config": {"skip_signature": True, "region": "us-east-2"}}

    _t0 = time.perf_counter()
    atlas = RaggedAtlas.checkout_latest(
        db_uri=HOMEOBOX_DIR + "lance_db",
        store_kwargs=S3_KWARGS,
    )
    homeobox_open_s = time.perf_counter() - _t0

    n_obs_homeobox = atlas.cell_table.count_rows()
    n_vars_homeobox = atlas._registry_tables["gene_expression"].count_rows()
    print(
        f"homeobox: {n_obs_homeobox:,} cells x {n_vars_homeobox:,} genes "
        f"(opened in {homeobox_open_s:.2f}s)"
    )
    return atlas, homeobox_open_s, n_obs_homeobox, n_vars_homeobox
...
def _(
    ...
    |---|---|---|
    | Cells | `{n_obs_tiledb:,}` | `{n_obs_homeobox:,}` |
    | Genes | `{n_vars_tiledb:,}` | `{n_vars_homeobox:,}` |
    | Open time | `{tiledb_open_s:.2f}s` | `{homeobox_open_s:.2f}s` |
    """)
    return
...
def _(
    ...
    for nw in worker_counts:
        ...
        # homeobox
        print(f"  homeobox     ...", end=" ", flush=True)
        res_l = run_homeobox_epoch(atlas, N_CELLS, BATCH_SIZE, SEED, num_workers=nw)
        print(
        ...
        multi_rows.append({
            ...
            "homeobox_epoch_s": round(res_l["total_s"], 2),
            "homeobox_cells_per_s": round(res_l["cells_per_s"]),
            "homeobox_med_batch_ms": round(res_l["med_batch_ms"], 1),
            "speedup": round(
...
def _(alt, mo, multi_worker_df, pl):
    ...
    _homeobox_plot = multi_worker_df.select(
        pl.col("workers"),
        pl.col("homeobox_cells_per_s").alias("cells_per_s"),
        pl.lit("homeobox").alias("system"),
    )
    _plot_df = pl.concat([_tiledb_plot, _homeobox_plot])
...
def _(mo, multi_worker_df):
    best_homeobox = multi_worker_df["homeobox_cells_per_s"].max()
    best_tiledb = multi_worker_df["tiledb_cells_per_s"].max()
    peak_speedup = best_homeobox / best_tiledb if best_tiledb > 0 else 0

    mo.md(f"""
    ### Dataloader Takeaways

    - **Peak homeobox throughput**: `{best_homeobox:,.0f}` cells/s
...
def _(atlas, experiment, n_genes_slider, np, pl):
    ...
    common_genes = sorted(
        set(homeobox_gene_reg["feature_name"].to_list()) & tiledb_var_names
    )
...
def _(mo):
    ...
    matrix. homeobox pushes the filter and limit into LanceDB; TileDB-SOMA
...
def _(
    ...
    def _run():
        ...
        for i in range(3):
            ...
            homeobox_times.append(time.perf_counter() - t0)
            print(f"  homeobox     run {i+1}: {homeobox_times[-1]:.2f}s -> {adata_l.shape}")
        ...
        l_med = float(np.median(homeobox_times))
        t_med = float(np.median(tiledb_times))
        return {
            "query": "Cell-oriented",
            "homeobox_shape": f"{adata_l.n_obs:,} x {adata_l.n_vars:,}",
            "homeobox_s": round(l_med, 2),
    ...
    mo.md(f"""
    | | homeobox | TileDB-SOMA |
    |---|---|---|
    | Shape | `{cell_query_result['homeobox_shape']}` | `{cell_query_result['tiledb_shape']}` |
    | Median time | `{cell_query_result['homeobox_s']}s` | `{cell_query_result['tiledb_s']}s` |
...
def _(mo):
    ...
    population. homeobox uses `.features(uids)` to filter by global feature
...
def _(
    ...
    for _i in range(N_RUNS):
        ...
        homeobox_times.append(time.perf_counter() - t0)
        print(f"  homeobox     run {_i+1}: {homeobox_times[-1]:.2f}s -> {adata_l.shape}")
...
def _(
    ...
    def _run():
        ...
        return homeobox_times, tiledb_times, adata_l, adata_t

    _homeobox_times, _tiledb_times, _adata_l, _adata_t = _run()

    _l_med = float(np.median(_homeobox_times))
    _t_med = float(np.median(_tiledb_times))
    comb_query_result = {
        "query": "Combined",
        "homeobox_shape": f"{_adata_l.n_obs:,} x {_adata_l.n_vars:,}",
        "homeobox_s": round(_l_med, 2),
...
def _(alt, cell_query_result, comb_query_result, feat_query_result, mo, pl):
    ...
    _homeobox_plot = query_df.select(
        pl.col("query"),
        pl.col("homeobox_s").alias("seconds"),
        pl.lit("homeobox").alias("system"),
    )
    _chart_df = pl.concat([_tiledb_plot, _homeobox_plot])
...
def _(cell_query_result, comb_query_result, feat_query_result, mo):
    ...
    homeobox's query API provides the same data access patterns as TileDB-SOMA
