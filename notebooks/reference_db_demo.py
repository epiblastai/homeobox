...
@app.cell
def _():
    from homeobox.standardization.metadata_table import (
        COMPOUNDS_TABLE,
...
@app.cell
def _():
    from homeobox.standardization.genes import (
...
def _(resolve_genes):
    symbol_report = resolve_genes(
        ["TP53", "BRCA1", "p53", "ACTB", "brca1", "NOTAREALGENE"],
        organism="human",
    )
...
def _(mo, symbol_report):
    ...
    **{symbol_report.ambiguous}** ambiguous.
    ...
    symbols like `TP53` get confidence 1.0. The typo `brac1` is caught by
    FTS fuzzy matching (confidence 0.8, source `lancedb_fts`).
    `NOTAREALGENE` correctly remains unresolved.
    """)
    return
...
@app.cell
def _():
    from homeobox.standardization.ontologies import (
        resolve_assays,
        resolve_cell_types,
        resolve_diseases,
        resolve_organisms,
...
def _(mo):
    ...
    synonyms, and FTS fuzzy matches (fuzziness=2) with varying confidence scores.
    """)
    return
...
def _(cell_type_report, mo):
    ...
    **{cell_type_report.resolved}** / **{cell_type_report.total}** resolved.
    ...
    Misspelled terms like `neruon` and `astrocye` are caught by FTS fuzzy
    matching (confidence 0.8, source `reference_db_fts`), while exact matches
    get confidence 1.0.
    """)
    return
...
def _(mo):
    ...
    on-demand access to 250+ ontologies.  The `homeobox.standardization.ols` module
...
@app.cell
def _():
    from homeobox.standardization.ols import (
        get_ols_ancestors,
        get_ols_mappings,
        get_ols_replacement,
        get_ols_term,
    ...
    from homeobox.standardization.ontologies import resolve_cell_lines
...
@app.cell
def _():
    from homeobox.standardization.proteins import resolve_proteins
...
def _(resolve_proteins):
    protein_report = resolve_proteins(
        ["TP53", "p53", "BRCA1", "insulin", "CD3D", "APP", "inslin", "NOTAPROTEIN"],
        organism="human",
    )
...
def _(mo, protein_report):
    ...
    **{protein_report.ambiguous}** ambiguous.
    ...
    The typo `inslin` is caught by FTS fuzzy matching (confidence 0.8,
    source `lancedb_fts`).
    """)
    return
...
@app.cell
def _():
    from homeobox.standardization.molecules import resolve_molecules
...
def _(mo, mol_report):
    ...
    **{mol_report.unresolved}** unresolved.
    ...
    Local DB matches show `source="lancedb"`. Typos like `dexamethason` and
    `imatanib` are caught by FTS fuzzy matching (confidence 0.8, source
    `lancedb_fts`) without hitting PubChem/ChEMBL APIs.
    """)
    return
