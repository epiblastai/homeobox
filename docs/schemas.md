...
| `dataset_uid` | `str` | Filled automatically by the ingestion layer to match the `dataset_uid` of the `DatasetRecord` the cell was ingested with. Do not set this manually. |
...
`DatasetRecord` is the dataset inventory table. One row is written per zarr group ingested into the atlas. `zarr_group` is the per-row primary key; `dataset_uid` is the logical dataset identifier that is shared across modality rows from the same multimodal batch (one row per feature space).
