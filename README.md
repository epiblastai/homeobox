...
Homeobox is a multimodal database for interactive analysis and ML training. It combines the search and versioning capabilities of [LanceDB](https://lancedb.com) with the array storage of [Zarr](https://zarr.dev).

A single homeobox atlas can hold sparse single-cell gene expression, dense protein and embedding features, 2D/3D/4D/5D images, biomolecular structures, and free text. A single dataloader streams batches across all of them without needing intermediate ML-specialized copies or modality-specific entrypoints. Our design goal is to be *extremely flexible, while still quite fast*.
