...
balancing).  :class:`~homeobox.dataloader.CellDataset` is left as a pure
...
try:
    ...
    class _TorchSampler:  # type: ignore[no-redef]
        ...
        def __init__(self, *args, **kwargs):
            raise ImportError(
                "torch is required for homeobox samplers. Install it with: pip install homeobox[ml]"
            )
...
class CellSampler(_TorchSampler):
    ...
    groups_np:
        ...
        :attr:`~homeobox.dataloader.CellDataset.groups_np`.
    batch_size:
        Cells per batch.
    shuffle:
