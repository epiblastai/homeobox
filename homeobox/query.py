"""AtlasQuery: fluent query builder for reading rows from a RaggedAtlas."""
...
class AtlasQuery:
    """Fluent query builder for reading rows from a RaggedAtlas."""

    def __init__(self, atlas: RaggedAtlas, obs_table_name: str | None = None) -> None:
        self._atlas = atlas
        name, table = atlas._resolve_obs_table(obs_table_name=obs_table_name)
        self._obs_table_name = name
    ...
    @property
    def pointer_field_names(self) -> list[str]:
        return list(self._pointer_fields.keys())

    @property
    def obs_table_name(self) -> str:
        """Name of the obs table this query targets."""
        return self._obs_table_name
    ...
    def balanced_limit(self, n: int, column: str) -> "AtlasQuery":
        """Limit rows, drawing equally from each unique value of *column*.

        The result contains at most *n* rows, split evenly across each
    ...
    def feature_spaces(self, *spaces: str) -> "AtlasQuery":
        ...
        """
        known = {pf.feature_space for pf in self._pointer_fields.values()}
    ...
    def select_fields(self, *field_names: str) -> "AtlasQuery":
        ...
        """
        known = set(self._pointer_fields.keys())
    ...
    def count(self, group_by: str | list[str] | None = None) -> "pl.DataFrame | int":
        ...
        -------
        int
            Total row count when ``group_by`` is ``None``.
        pl.DataFrame
        ...
        if group_by is None:
            ...
            any_col = self._obs_table.schema.names[0]
    ...
    def to_multimodal(self) -> "MultimodalResult":
        ...
        for pf in active_pfs.values():
            ...
            if "as_fragments" in endpoints:
                result = reconstructor.as_fragments(
                    self._atlas,
                    obs_pl,
                    pf,
                )
    ...
    def to_batches(self, batch_size: int = 1024) -> Iterator[ad.AnnData]:
        ...
        Each batch contains up to ``batch_size`` rows. BatchArray readers
    ...
    def to_multimodal_dataset(
        ...
        one sub-batch per modality with only the rows that have that
