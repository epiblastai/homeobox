...
class ExternalDataloaderBenchmark:
    ...
    def __init__(
        ...
        self.console.print(f"Loaded {self.adata.n_obs:,} cells, {self.adata.n_vars:,} genes")
        ...
        with self.create_progress_bar(f"Training: {system_name}", "training") as pbar:
            for _batch in dataloader:
                ...
                if elapsed >= warmup_duration and not measurement_started:
                    ...
                    self.console.print(f"  Starting measurement after {warmup_duration}s warmup...")
                ...
                current_throughput = (
                    total_cells / (elapsed - warmup_duration) if elapsed > warmup_duration else 0
                )
    ...
    def benchmark_slaf_tier1(self) -> ExternalBenchmarkResult:
        ...
        total_cells, batch_count, elapsed_time, peak_memory = self.benchmark_with_memory_tracking(
        ...
        throughput_cells_per_sec = total_cells / measurement_time if measurement_time > 0 else 0
    ...
    def print_results(self, results: list[ExternalBenchmarkResult]):
        ...
        if slaf_tier1 and slaf_tier2:
            ...
            self.console.print(f"✓ SLAF Token Throughput: {slaf_tokens_per_sec:,.0f} tokens/sec")

            # Compare with scDataset
            scdataset_tier1 = next((r for r in tier1_results if r.system_name == "scDataset"), None)
            if scdataset_tier1:
                advantage = (
                    slaf_tier1.throughput_cells_per_sec / scdataset_tier1.throughput_cells_per_sec
                    - 1
                ) * 100
                self.console.print(f"✓ SLAF vs scDataset Tier 1: {advantage:+.1f}% advantage")
