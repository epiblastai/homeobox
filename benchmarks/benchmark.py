...
DEFAULT_DATASET_DIR = str((Path(__file__).parent.parent.parent / "slaf-datasets").resolve())
            ...
            if results:
                ...
                if results and len(results) > 0:
                    ...
                    if "slaf_vs_h5ad_speedup" in first_result:
                        ...
                        print(f"  Average SLAF vs h5ad speedup: {avg_slaf_vs_h5ad:.1f}x")
                        print(f"  Average SLAF vs TileDB speedup: {avg_slaf_vs_tiledb:.1f}x")
                    else:
...
def extract_cell_filtering_summary(results: list[dict]) -> dict[str, Any]:
    ...
    if "slaf_vs_h5ad_speedup" in first_result:
        ...
        # Two-way comparison (legacy)
        total_speedups = [r["total_speedup"] for r in results if r.get("total_speedup", 0) > 0]
        ...
        representative_scenarios.append(
            {
                ...
                "memory_efficiency": (round(h5ad_mem / slaf_mem, 1) if slaf_mem > 0.1 else "N/A"),
            }
        )
...
def generate_benchmark_summary(input_file: str, output_file: str):
    ...
    for dataset_name, dataset_results in comprehensive_results.items():
        ...
        if "expression_queries" in dataset_results:
            summary[dataset_name]["expression_queries"] = extract_expression_queries_summary(
...
def update_performance_docs(summary_file: str, docs_file: str):
    ...
    if "cell_filtering" in dataset_summary:
        content = update_cell_filtering_section(content, dataset_summary["cell_filtering"])

    if "gene_filtering" in dataset_summary:
        content = update_gene_filtering_section(content, dataset_summary["gene_filtering"])

    if "expression_queries" in dataset_summary:
        content = update_expression_queries_section(content, dataset_summary["expression_queries"])

    # Write updated content
    with open(docs_file, "w") as f:
        f.write(content)

    print(f"✅ Updated {docs_file} with actual benchmark numbers and three-way comparison")
