...
class EntropyCalculator:
    ...
    def _compute_baselines(self):
        ...
        random_ids = np.random.choice(self.max_num_cells, size=batch_size, replace=False)
        ...
        sequential_batches = [list(range(i * batch_size, (i + 1) * batch_size)) for i in range(10)]
        self.sequential_across_l1 = self._compute_across_batch_l1(sequential_batches)

        # Random: batches of random IDs
        random_batches = [
            np.random.choice(self.max_num_cells, size=batch_size, replace=False).tolist()
            for _ in range(10)
        ]
    ...
    def _compute_across_batch_l1(self, all_batch_ids: list[list[int]]) -> float:
        ...
        for i in range(len(all_batch_ids) - 1):
            ...
            try:
                ...
                indices1 = np.random.choice(len(batch1), size=n_cell_pairs, replace=False)
                indices2 = np.random.choice(len(batch2), size=n_cell_pairs, replace=False)
    ...
    def compute_within_batch_entropy(self, batch_ids: list[list[int]]) -> dict[str, float]:
    ...
    def compute_across_batch_entropy(self, all_batch_ids: list[list[int]]) -> dict[str, float]:
...
class EntropyBenchmark:
    ...
    def create_progress_bar(self, desc: str, mode: str = "training") -> tqdm:
        ...
        if mode == "prefetch":
            ...
            bar_format = (
                "{l_bar}{bar}| {n_fmt} batches [{elapsed}<{remaining}, {rate_fmt}] {postfix}"
            )

        return tqdm(desc=desc, unit="batch", bar_format=bar_format, ncols=100, leave=False)
    ...
    def run_entropy_only_measurement(
        ...
        dataloader = SLAFDataLoader(
            ...
            batches_per_chunk=(50 if not use_mos else 1),  # 50 for sequential/fragment, 1 for MoS
        )
        ...
        print(f"    Collected {len(all_batch_ids)} batches in {entropy_measurement_time:.2f}s")
        ...
        across_batch_metrics = self.entropy_calc.compute_across_batch_entropy(all_batch_ids)
    ...
    def run_throughput_only_measurement(
        ...
        with self.create_progress_bar(f"Throughput: {strategy_name} strategy", "training") as pbar:
            for batch in dataloader:
                ...
                current_throughput = throughput_total_cells / elapsed if elapsed > 0 else 0
        ...
        throughput_cells_per_sec = (
            throughput_total_cells / throughput_elapsed_time if throughput_elapsed_time > 0 else 0
        )
    ...
    def run_entropy_measurement(
        ...
        print(f"  - Collected {len(all_batch_ids)} batches with {total_cells} total cells")
    ...
    def print_combined_results(self, results: list[EntropyStrategyResult]):
        ...
        for result in strategy_results:
            ...
            sequential_result = next(r for r in strategy_results if r.strategy == "sequential")
