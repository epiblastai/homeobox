...
def demo_realistic_cell_queries():
    ...
    scenarios = [
        ...
        # Cluster-based filtering (common after clustering)
        {
            ...
            "h5ad_code": lambda adata: adata.obs[adata.obs.leiden.isin(["0", "1", "2"])],
        ...
        # Combined filtering (most realistic)
        {
            ...
            "slaf_code": lambda slaf: slaf.filter_cells(leiden=["0", "1"], batch="batch_1"),
        ...
        },
        {
            ...
            "h5ad_code": lambda adata: adata.obs[
                (adata.obs.n_genes_by_counts >= 200) & (adata.obs.n_genes_by_counts <= 1500)
            ],
            "slaf_code": lambda slaf: slaf.filter_cells(n_genes_by_counts=">=200").filter(
...
def benchmark_cell_filtering_scenario(
    ...
    tiledb_total_time = tiledb_result["tiledb_load_time"] + tiledb_result["tiledb_query_time"]

    # Calculate speedups - focus on SLAF comparisons
    slaf_vs_h5ad_speedup = h5ad_total_time / slaf_total_time if slaf_total_time > 0 else 0
    slaf_vs_tiledb_speedup = tiledb_total_time / slaf_total_time if slaf_total_time > 0 else 0
    ...
    return {
        ...
        "results_match": (
            h5ad_result["h5ad_count"] == slaf_result["slaf_count"] == tiledb_result["tiledb_count"]
        ),
        # Memory breakdown
        "h5ad_load_memory_mb": h5ad_result["h5ad_load_memory"],
        "h5ad_query_memory_mb": h5ad_result["h5ad_query_memory"],
        "h5ad_total_memory_mb": h5ad_result["h5ad_load_memory"] + h5ad_result["h5ad_query_memory"],
        "slaf_load_memory_mb": slaf_result["slaf_init_memory"],
        "slaf_query_memory_mb": slaf_result["slaf_query_memory"],
        "slaf_total_memory_mb": slaf_result["slaf_init_memory"] + slaf_result["slaf_query_memory"],
...
def benchmark_cell_filtering(
    ...
    for i, scenario in enumerate(scenarios):
        ...
        try:
            ...
            result = benchmark_cell_filtering_scenario(h5ad_path, slaf_path, tiledb_path, scenario)
