...
class scDatasetMultiprocessingBenchmark:
    ...
    def benchmark_with_memory_tracking(self, dataloader, measurement_duration: float, desc: str):
    ...
    def benchmark_scdataset_multiprocessing(
        ...
        try:
            ...
            total_cells, batch_count, elapsed_time, peak_memory = (
                self.benchmark_with_memory_tracking(dataloader, self.measurement_duration, desc)
            )
            ...
            self.console.print(f"[red]Error benchmarking num_workers={num_workers}: {e}[/red]")
            return None
    ...
    def run_benchmarks(self) -> list[MultiprocessingBenchmarkResult]:
        ...
        for num_workers in self.num_workers_list:
            ...
            if result:
                ...
                self.console.print(f"  Result: {result.throughput_cells_per_sec:.0f} cells/sec")
    ...
    def print_results(self, results: list[MultiprocessingBenchmarkResult]):
        ...
        if results:
            ...
            best_config = next(r for r in results if r.throughput_cells_per_sec == max_throughput)

            self.console.print("\n[bold]Summary:[/bold]")
            self.console.print(f"Best performance: {max_throughput:.0f} cells/sec")
            self.console.print(f"Best configuration: num_workers={best_config.num_workers}")

            # Calculate scaling factor
            single_worker_result = next((r for r in results if r.num_workers == 0), None)
            if single_worker_result and best_config.num_workers > 0:
                ...
                self.console.print(f"Multiprocessing scaling factor: {scaling_factor:.1f}x")
