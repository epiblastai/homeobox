...
def print_benchmark_table(results: list[dict], dataset_name: str = "", scenario_type: str = ""):
    ...
    if has_tiledb:
        ...
        slaf_vs_tiledb_speedups = [
            r["slaf_vs_tiledb_speedup"] for r in results if r["slaf_vs_tiledb_speedup"] > 0
        ]

        avg_slaf_vs_h5ad_speedup = np.mean(slaf_vs_h5ad_speedups) if slaf_vs_h5ad_speedups else 0
        ...
        if memory_results:
            avg_h5ad_total_memory = np.mean([r["h5ad_total_memory_mb"] for r in memory_results])
            avg_slaf_total_memory = np.mean([r["slaf_total_memory_mb"] for r in memory_results])
            avg_tiledb_total_memory = np.mean([r["tiledb_total_memory_mb"] for r in memory_results])
            memory_efficiency = (
                avg_h5ad_total_memory / avg_slaf_total_memory if avg_slaf_total_memory > 0 else 0
            )
        else:
...
def _print_rich_table(
    ...
    for i, result in enumerate(results):
        ...
        if slaf_total_memory > 0.01:  # Lower threshold to catch small but meaningful differences
...
def warm_up_tiledb_database(experiment, verbose=False):
    ...
    warmup_operations = [
        ...
        lambda: experiment.ms["RNA"]["X"]["data"].read((slice(0, 10),)).tables().concat(),
        ...
        lambda: experiment.ms["RNA"]["X"]["data"].read((slice(0, 20),)).tables().concat(),
    ...
    for i, operation in enumerate(warmup_operations):
        if verbose and i % 3 == 0:  # Print progress every 3 operations
            console.print(f"    Warming up operation {i + 1}/{len(warmup_operations)}...")
...
def run_with_burn_in(scenarios, benchmark_func, slaf_instance=None, verbose=False, **kwargs):
...
def _print_rich_table_three_way(
    ...
    for i, result in enumerate(results):
        ...
        slaf_vs_h5ad_text = f"{slaf_vs_h5ad_speedup:.1f}x" if slaf_vs_h5ad_speedup > 0 else "N/A"
        slaf_vs_tiledb_text = (
    ...
    summary_table.add_row("Average SLAF vs h5ad Speedup", f"{avg_slaf_vs_h5ad_speedup:.1f}x")
    summary_table.add_row("Average SLAF vs TileDB Speedup", f"{avg_slaf_vs_tiledb_speedup:.1f}x")
    ...
    summary_table.add_row("Average TileDB Memory (MB)", f"{avg_tiledb_total_memory:.1f}")
    ...
    console.print(f"• SLAF vs h5ad: {avg_slaf_vs_h5ad_speedup:.1f}x faster than traditional h5ad")
    console.print(f"• SLAF vs TileDB: {avg_slaf_vs_tiledb_speedup:.1f}x faster than TileDB SOMA")
    console.print(f"• Memory efficiency: SLAF uses {memory_efficiency:.1f}x less memory than h5ad")
