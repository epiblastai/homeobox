...
def extract_h5ad_obs_var(h5ad_path: str) -> tuple[str, str]:
    ...
    (cell barcodes for obs, feature ids for var). Only the obs and var groups are
    read, so neither X nor any layer is loaded into memory -- backed mode is not
    enough here, since it still reads layers, obsm, obsp, and varp eagerly.
    Returns ``(obs_csv_path, var_csv_path)``.
    """
    ...
    with h5py.File(h5ad_path, "r") as handle:
        read_elem(handle["obs"]).to_csv(obs_csv_path)
        read_elem(handle["var"]).to_csv(var_csv_path)
